...
export function Header({ onMenuToggle }: { onMenuToggle?: () => void }) {
  ...
  const getPageTitle = () => {
    ...
    if (pathname.startsWith('/subscribers') || pathname.startsWith('/contacts')) return 'KİŞİLER & REHBER';
    if (pathname.startsWith('/groups')) return 'GRUP YÖNETİMİ';
    if (pathname.startsWith('/templates')) return 'MESAJ ŞABLONLARI';
    if (pathname.startsWith('/campaigns')) return 'TOPLU MESAJ GÖNDERİMİ';
    if (pathname.startsWith('/users')) return 'KULLANICI YÖNETİMİ';
    if (pathname.startsWith('/settings')) return 'SİSTEM AYARLARI';
    return 'WHATSPULSE';
  };
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="flex items-center gap-2.5">
              ...
              <button
                ...
                title="Sürüm Notları & Değişiklik Günlüğü"
