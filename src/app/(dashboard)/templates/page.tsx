...
export default function TemplatesPage() {
  ...
  const categories = [
    { id: 'all', label: 'Tüm Şablonlar', icon: Layers },
    { id: 'dini', label: '🌿 Dini Günler & Tebrikler', icon: Moon },
    { id: 'resmi', label: '🇹🇷 Resmi Bayramlar', icon: Flag },
    { id: 'genel', label: '✨ Genel & Kampanyalar', icon: Sparkles },
  ];
  ...
  const handleSaveTemplate = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setForm({ name: '', category: 'Genel', content: '', mediaType: 'text', mediaUrl: '', variables: [] });
        loadTemplates();
      }
    } catch (err) {
      console.error(err);
  ...
  const filteredTemplates = templates.filter((t) => {
    ...
    if (selectedCategory === 'dini') {
      return t.name.includes('Cuma') || t.name.includes('Ramazan') || t.name.includes('Kurban') || t.name.includes('Kandil') || t.category === 'Dini Günler';
    }
    if (selectedCategory === 'resmi') {
      return t.name.includes('Cumhuriyet') || t.name.includes('Zafer') || t.name.includes('Nisan') || t.category === 'Resmi Bayramlar';
    }
    if (selectedCategory === 'genel') {
      return !t.name.includes('Cuma') && !t.name.includes('Ramazan') && !t.name.includes('Kurban') && !t.name.includes('Kandil') && !t.name.includes('Cumhuriyet') && !t.name.includes('Zafer') && !t.name.includes('Nisan');
    }
    return true;
  });
  ...
  return (
    <div className="space-y-6 animate-fade-in pb-12">
        ...
        <div>
          <div className="flex items-center gap-2">
            <span className="px-2.5 py-0.5 rounded-full bg-[#d4af37]/15 text-[#d4af37] text-xs font-bold border border-[#d4af37]/30 font-serif-title">
              ŞABLON MERKEZİ & KÜTÜPHANE
            </span>
          </div>
          <h1 className="text-xl sm:text-2xl font-black text-white mt-1 font-serif-title">
            Mesaj Şablonları & Hazır Tebrik Kütüphanesi
          </h1>
          <p className="text-xs text-gray-400">
            Dini günler (Cuma, Bayramlar, Kandil) ve Resmi bayramlar için hazır mesajları kullanın veya özel şablonlar tanımlayın.
          </p>
        </div>
      ...
      <div className="p-4 rounded-3xl bg-[#121517] border border-[#23292e] space-y-3 shadow-md">
        {/* Category Tabs */}
        <div className="flex flex-wrap items-center gap-2 pb-1 border-b border-[#23292e]/60">
          {categories.map((cat) => {
            ...
            return (
              <button
                ...
                className={`px-3.5 py-2 rounded-xl text-xs font-bold transition-all cursor-pointer flex items-center gap-1.5 ${
                  isSelected
                    ? 'bg-gradient-to-r from-[#d4af37] to-[#b89528] text-black shadow-md shadow-[#d4af37]/20'
                    : 'bg-[#181c1f] text-gray-300 hover:bg-[#202529] hover:text-white border border-[#2e353c]'
                }`}
              >
                <span>{cat.label}</span>
              </button>
      ...
      {loading && templates.length === 0 ? (
        ...
        <div className="p-12 text-center text-gray-500 bg-[#121517] border border-[#23292e] rounded-3xl space-y-3">
          ...
          <h3 className="text-sm font-bold text-gray-300 font-serif-title">Şablon Bulunamadı</h3>
          <p className="text-xs text-gray-500 max-w-sm mx-auto">
            Arama kriterlerinize uyan şablon bulunamadı.
          </p>
        </div>
      ) : (
            ...
            >
              <div className="space-y-3">
                <div className="flex items-center justify-between">
                  <span className="text-xs font-bold text-[#d4af37] uppercase tracking-wider font-serif-title flex items-center gap-1.5 truncate max-w-[200px]">
                    <Bookmark className="w-3.5 h-3.5 text-[#d4af37] shrink-0" />
                    <span className="truncate">{t.name}</span>
                  </span>
                  <span className="text-[10px] px-2 py-0.5 rounded-full bg-[#10b981]/15 text-[#10b981] border border-[#10b981]/30 font-mono font-bold shrink-0">
              ...
              <div className="flex items-center justify-between pt-3 border-t border-[#23292e]/60">
                <Link
                  ...
                  className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-[#10b981]/15 hover:bg-[#10b981]/25 text-[#10b981] border border-[#10b981]/30 text-xs font-bold transition-all"
                >
            ...
            <form onSubmit={handleSaveTemplate} className="space-y-4">
              ...
              <div>
                ...
                <textarea
                  required
                  rows={5}
                  placeholder="Merhaba {ad} {soyad}, WhatsPulse üzerinden size özel hazırlanan tebriğimiz..."
                  value={form.content}
